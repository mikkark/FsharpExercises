...
module rakkaudenFastTrack =
    ...
    let rec findMatches(names:string list, namestomatchto:string list, res) =
        match names.Length with
            | 0 -> res
            | _ -> findMatches(names.Tail, namestomatchto.Tail, matchWithName (namestomatchto, names.Head, res))
            (0, 200); (200, 200); (400, 200); (600, 200); (800, 201)
    Async.Parallel [for x in [ for y in 0..5 do yield y ] ] ->  
                                        async { let namesToMatch = names |> Seq.skip (fst x) |> Seq.take (snd x) |> Seq.toList
                                                let namesToMatchTo = names |> Seq.skip (fst x) |> Seq.toList
                                                let res = findMatches (namesToMatch, namesToMatchTo, []) 
                                                return res } ]
    |> Async.RunSynchronously
    |> Array.collect (fun setOfPairs -> setOfPairs |> List.toArray)
    |> fun arr -> printfn "%d" arr.Length

//System.IO.File.WriteAllLines(@"C:\Omat projektit\Fsharpexercises\Exercises\fast_track_parit_parallel.txt", resHuh)

//    let s1 = getCharCounts "Zacharias Sillanpää"
//    let s2 = getCharCounts "Velipekka Sinikantele"
//    let paired = sumCharCountsAsList s1 s2
//
//    printfn "%A" (calculatePAIRS paired)
