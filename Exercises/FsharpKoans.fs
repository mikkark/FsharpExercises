...
module FindBiggestOpenCloseVariation =
    ...
    let asArraysOfValues =
        Seq.skip 1 stockData
        |> Seq.toList
        |> List.map (fun line -> line.Split[|'|'|])
        |> List.maxBy(fun day -> abs(System.Double.Parse(day.[1]) - System.Double.Parse(day.[4])))

    for line in asArraysOfValues do
        printfn "%s" line
